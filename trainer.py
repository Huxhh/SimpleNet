...
class Trainer:
    def __init__(self, network, x_train, t_train, x_test,
                 t_test, epochs=10, batch_size=32, optimizer='SGD',
                 optimizer_params={'lr': 0.01}, evaluate_sample_num_per_epoch=None,
                 verbose=True):
    ...
    def train_step(self):
        ...
        # x_batch = x_batch.reshape(32, 1, 28, 28)
        grads = self.network.gradient(x_batch, t_batch)
        self.optimizer.update(self.network.params, grads)

        loss = self.network.loss(x_batch, t_batch)
        self.train_loss_dict.append(loss)
        if self.verbose:
